...
3. **`broker.TaskBroker`** — `github.com/BabySid/aether/broker`
   ...
   - Implement a single Gaia-side Redis broker; the pinned interface exchanges `wire.TaskAssignment` and `wire.TaskResult`. See the initial broker design below.
...
3. Implement `broker.TaskBroker` using the initial broker design below for separately deployed Workers.
...
## Initial broker design

Gaia hosts one Redis-backed broker constructed with `NewBroker`, a Redis client, a configured key prefix, and required start/completion handlers. Composition code passes closures that invoke `Engine.OnTaskStarted` and `Engine.OnTaskCompleted` once the Engine is constructed. Workers do not call the Go broker directly; transport/API design is separate from this implementation.

`Dispatch` validates a non-nil assignment with non-empty task-run ID, workflow-run ID, and executor type, then adds the JSON-serialized `wire.TaskAssignment` as a member of an executor-specific Redis ZSet. The score is `Priority`: higher scores are fetched first, and order among equal scores is unspecified. Redis coalesces identical JSON members while they remain queued; distinct assignments with the same task-run ID are not rejected, and retry dispatch after fetch can queue the same assignment again. A length-framed key prefix separates deployments even when prefix and executor names contain colons. `FetchTask` parses worker IDs as `v1::<identifier>::<executor_type>` (non-empty components, no embedded `::`), uses the executor type to select the queue, checks `ZCARD`, reads the highest-scored member, then calls `ZREM`. Return the decoded assignment only when `ZREM` removes one member; otherwise return a Gaia-defined `ErrNoTaskAvailable` immediately. Do not use Lua or add blocking/polling inside the broker. Concurrent fetchers may race between commands; the successful `ZREM` decides who receives the assignment. Redis errors are not treated as an empty queue.

`StartTask` and `CompleteTask` invoke the injected Engine handlers directly. The Engine callbacks return no errors; their internal failures are not exposed to callers by these methods. `Cancel` returns Gaia's `ErrNotImplemented` when the broker is open and the context is active; a closed broker returns `ErrClosed`, and an already-canceled context returns its error. `Close` rejects new operations and releases broker-owned resources; it does not yet wait for already-fetched Worker tasks to drain. Mark this and lease/redelivery recovery as TODOs at the relevant implementation sites. In particular, a Worker crash after fetch can lose an assignment: initial delivery is at-most-once, not at-least-once. Redis is assumed highly available, but this does not eliminate the gap between Store and Redis: the pinned Engine writes Ready before `Dispatch`, and `Start()` does not replay those tasks. Do not reverse the operations in Gaia; resolving cross-system recovery is separate work.

This initial design intentionally deviates from the pinned `FetchTask` comment's blocking/context-expiry behavior and `Close` comment's drain guarantee. Aether's retry and timeout paths still exist: retries may dispatch the same task-run ID again, and timeout/cancellation paths may invoke unsupported `Cancel`. Do not describe either path as providing execution cancellation or recovery in this version.
